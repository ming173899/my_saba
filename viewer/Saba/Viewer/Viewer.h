...
namespace saba
{
	...
	private:
		...
		bool ChangeMMD();
		void ChangeCamera();
		void SaveDataSet(double lefteye, double righteye, double mouse, int x, int y, int z, std::string filename);
	...
	private:
		...
		double x2, y2;
		int tabCount = 0;
		unsigned long int MMDNumbers = 0;

		int operatingModes = 0;
		int cameraModes = 0;

		bool cameraListSwitch = false;

		std::vector<std::string> MMDNames;
		std::vector<std::string> MMDCameras;
