...
void screenShot(int x, int y, int w, int h, std::string filename) {
	...
	unsigned char* temp = new unsigned char[w * 4];
	...
	*/
	...
	image = FreeImage_ConvertFromRawBits(data, w, h, w * 4, 32, 0, 0, 0);
	...
	if (FreeImage_Save(FIF_PNG,image, filename.c_str(), PNG_DEFAULT))
...
namespace saba
{
	...
	Viewer::InitializeParameter::InitializeParameter()
		...
		, m_msaaCount(4)
		, m_initCamera(false)
		, m_initCameraCenter(0, 0, 0) //x,y,z coordinate
		, m_initCameraEye(0, 0, 10) //x,y,z coordinate
	...
	const glm::vec3 Viewer::DefaultBGColor1 = glm::vec4(0.0f,0.0f,0.0f,0);
	const glm::vec3 Viewer::DefaultBGColor2 = glm::vec4(0.0f,0.0f,0.0f,0);
	...
	bool Viewer::Initialize(const InitializeParameter& initParam)
	{
		...
		m_window = glfwCreateWindow(256, 256, "Saba Viewer", nullptr, nullptr);
		...
		m_context.m_camera.Initialize(glm::vec3(0), 10.0f, cameraModes);
	...
	int Viewer::Run()
	{
		while (!glfwWindowShouldClose(m_window))
		{
			...
			if (m_mouseLockMode == MouseLockMode::Lock && m_cameraMode != CameraMode::None)
			{
				...
				{
					std::cout << m_mouse.m_dx << "," << m_mouse.m_dy << std::endl;
	...
	void Viewer::DrawInfoUI()
	{
		...
		ImGui::SetNextWindowPos(ImVec2(1200, 30));
		if (!ImGui::Begin("Info", &m_enableCommandUI, ImGuiWindowFlags_NoTitleBar  | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_AlwaysAutoResize)) //ImGuiWindowFlags_NoResize
		{
			ImGui::End();
			return;
		}
	...
	void Viewer::DrawLogUI()
	{
		...
		ImGui::SetNextWindowPos(
			ImVec2(1200, (float)(m_context.GetWindowHeight()) - height - 80),
	...
	void Viewer::DrawCommandUI()
	{
		...
		ImGui::SetNextWindowSize(ImVec2(width, height), ImGuiCond_FirstUseEver);
		ImGui::SetNextWindowPos(
			ImVec2(1200, (float)m_context.GetWindowHeight() - height - 60),
			ImGuiCond_FirstUseEver
		);
	...
	void Viewer::DrawCtrlUI()
	{
		...
		ImGui::SetNextWindowPos(ImVec2(1200, 100 + 20), ImGuiCond_Once);
	...
	bool Viewer::CmdOpen(const std::vector<std::string>& args)
	{
		...
		else if (ext == "txt") 
		{
			...
			//std::ifstream ifs("D:\\mypaper\\pmxList3.txt", std::ios::in);
			if (!ifs.is_open()) {
				...
				while (1)
				{
					next = filepath.find_first_of("\\", current);
					if (next != current)
					{
						std::string tmp = filepath.substr(current, next - current);
						if (tmp.size() != 0) //忽略空字串
							tmpString = tmp;
					}
				...
				if (tmpString == "CameraList.txt") {
					cameraListSwitch = true;
					MMDCameras.clear();
					std::string s;
					while (std::getline(ifs, s)) {
						MMDCameras.push_back(s);
					}
					MMDNumbers = MMDCameras.size();
					ChangeMMD();
				}
				else {
					MMDNames.clear();
	...
	void Viewer::InitializeCamera()
	{
		...
		{
			if (cameraModes == 0) {
				auto center = glm::vec3(0, 0, 40);
				auto radius = glm::length(m_bboxMax - center);
				m_context.m_camera.Initialize(center, radius, cameraModes);
			}
			...
			m_context.m_camera.Initialize(-5,5,15,25);
		}
	}
	...
	void Viewer::OnMouseButtonStub(GLFWwindow * window, int button, int action, int mods)
	{
		...
		if (viewer != nullptr)
		{
			viewer->OnMouseButton(window,button, action, mods);
		}
	...
	void Viewer::OnMouseButton(GLFWwindow* window,int button, int action, int mods)
	{
		...
		else if (operatingModes == 1) {
			if (button == GLFW_MOUSE_BUTTON_LEFT && action == GLFW_PRESS && tabCount == 0) {
				glfwGetCursorPos(window, &x1, &y1);
				/*std::cout << "x1:" << x1 << ",y1:" << y1 << std::endl;
				std::cout << "x2:" << x2 << ",y2:" << y2 << std::endl;*/
				tabCount++;
			}
			else if (button == GLFW_MOUSE_BUTTON_LEFT && action == GLFW_PRESS && tabCount == 1) {
				...
				float left = m_context.m_camera.GetOrthoLeft();
				float right = m_context.m_camera.GetOrthoRight();
				float button = m_context.m_camera.GetOrthoButton();
				float top = m_context.m_camera.GetOrthoTop();
				
				std::cout << left << "," << right << "," << button << "," << top << std::endl;

				
				float new_button = top - ((top - button) * (y2 / h));
				float new_top = top - ((top - button)*(y1 / h));

				float temp = (new_top - new_button)/2;
				new_button -= temp;
				new_top += temp;

				float new_left = left+((right - left) * (x1 / w)) - temp * 2;
				float new_right = left+((right - left) * (x1 / w)) + temp * 2;

				m_context.m_camera.Initialize(new_left, new_right, new_button, new_top);

				std::cout << new_left << "," << new_right << "," << new_button << "," << new_top << std::endl;

				glm::vec3 center = m_context.m_camera.GetCenterPostion();
				glm::vec3 eye = m_context.m_camera.GetEyePostion();

				std::string tempString = std::to_string(center[0]) + " " + std::to_string(center[1]) + " " + std::to_string(center[2]) + " " +
					" " + std::to_string(eye[0]) + " " + std::to_string(eye[1]) + " " + std::to_string(eye[2]) + 
					" " + std::to_string(new_left) + " " + std::to_string(new_right) + " " + std::to_string(new_button) + " " + std::to_string(new_top);
				
				MMDCameras.push_back(tempString);
	...
	void Viewer::OnKey(int key, int scancode, int action, int mods)
	{
		...
		if (key == GLFW_KEY_E && action == GLFW_PRESS) {
			std::cout << "E: ";
			if (operatingModes == 0) {
				std::cout << "Data Produce Mode Open" << std::endl;
				//std::cout << "operatingModes==1" << std::endl;
				operatingModes++;
			}
			else if (operatingModes == 1) {
				std::cout << "Data Produce Mode Close" << std::endl;
				//std::cout << "operatingModes==0" << std::endl;
				operatingModes = 0;
			}
		}
		if (key == GLFW_KEY_C && action == GLFW_PRESS) {
			std::cout << "C: Camera Mode ";
			if (cameraModes == 0) {
				std::cout << "Perspective" << std::endl;
				//std::cout << "cameraModes==1" << std::endl;
				cameraModes++;
				InitializeScene();
			}
			else if (cameraModes == 1) {
				std::cout << "Ortho" << std::endl;
				//std::cout << "cameraModes==0" << std::endl;
				cameraModes = 0;
				InitializeScene();
			}
		}
		if (key == GLFW_KEY_S && action == GLFW_PRESS) {			//Save CameraList
			if (MMDCameras.size() != 0 && cameraListSwitch == true) {
				std::cout << "S: Save" << std::endl;
				std::string filename("d:\\mypaper\\CameraList.txt");
				std::fstream ifs;
				ifs.open(filename, std::ios_base::out);
				if (!ifs.is_open()) {
					std::cout << "failed to open " << filename << '\n';
				}
				else {
					for (auto& s : MMDCameras) {
						ifs << s << std::endl;
					}
					//ifs.write(text.data(), text.size());
					std::cout << "Done Writing!" << std::endl;
				}
			}
		}
		if (key == GLFW_KEY_BACKSPACE && action == GLFW_PRESS) {	//Remove CameraList At Last
			std::cout << "BackSpace: Delete Set In" << std::endl;
			MMDCameras.pop_back();
		}
		...
		if (key == GLFW_KEY_P && action == GLFW_PRESS) {			//Take Picture
			...
			std::string imgBaseString = "D:\\mypaper\\dataSet\\img";
			std::string labelBaseString = "D:\\mypaper\\dataSet\\label";

			for (int i = 0; i < MMDNames.size(); i++) {

				if (0 != access((imgBaseString + "\\" + std::to_string(i)).c_str(), 0))
				{
					mkdir((imgBaseString + "\\" + std::to_string(i)).c_str());
				}

				if (0 != access((labelBaseString + "\\" + std::to_string(i)).c_str(), 0))
				{
					mkdir((labelBaseString + "\\" + std::to_string(i)).c_str());
				}
			...
			for (; MMDNumbers < MMDCameras.size(); MMDNumbers++) {
				...
				std::mt19937 gen(rd());
				std::uniform_real_distribution<double> unif(0.0, 1.01);
				auto randfun = std::bind(unif, gen);

				for (int i = 0; i < 70; i++) {

					lefteye.push_back(randfun());
					righteye.push_back(randfun());
					mouse.push_back(randfun());
				}

				std::vector<int> range{ -16, 0, 16 };
				std::vector<int> weight{ 85, 15, 85 };
				std::piecewise_linear_distribution<> d(range.begin(), range.end(), weight.begin());
	...
	bool Viewer::ChangeMMD()
	{
		if (MMDNumbers < MMDNames.size() && MMDNumbers>=0) {
			if (!m_modelDrawers.empty()) {
				m_modelDrawers.clear();
			}
			std::string tempext = PathUtil::GetExt(MMDNames[MMDNumbers]);
			InitializeAnimation();
			if (tempext == "pmx") {
				if (!LoadPMXFile(MMDNames[MMDNumbers]))
				{
					return false;
				}
			}
			else if (tempext == "pmd") {
				if (!LoadPMDFile(MMDNames[MMDNumbers]))
				{
					return false;
				}
			}
			std::cout << MMDNumbers + 1 << std::endl;
		}
	}

	void Viewer::ChangeCamera() {

		int current = 0; //最初由 0 的位置開始找
		int next;
		std::vector<double> tmpDouble;
		std::string filepath = MMDCameras[MMDNumbers];
		...
		while (1)
		{
			...
			if (next != current)
			{
				std::string tmp = filepath.substr(current, next - current);
				if (tmp.size() != 0) //忽略空字串
					tmpDouble.push_back(std::stod(tmp));
			}
			if (next == std::string::npos) break;
			current = next + 1; //下次由 next + 1 的位置開始找起。
		}

		std::cout << tmpDouble[2] << std::endl;

		auto center = glm::vec3(tmpDouble[0], tmpDouble[1], tmpDouble[2]);
		auto radius = glm::length(m_bboxMax - center);
		m_context.m_camera.Initialize(center, radius, cameraModes);
		std::cout << center[0] << "," << center[1] << "," << center[2] << std::endl;
		m_context.m_camera.Initialize(tmpDouble[6], tmpDouble[7], tmpDouble[8], tmpDouble[9]);
		std::cout << tmpDouble[6] << "," << tmpDouble[7] << "," << tmpDouble[8] << "," << tmpDouble[9] << std::endl;;
	}

	void Viewer::SaveDataSet(double lefteye, double righteye, double mouse, int x, int y, int z, std::string filename) {

		std::ofstream fout(filename);
		if (fout) { // 如果建立成功
			
			std::cout << "Create " << filename << " sucess!!" << std::endl;

			fout << std::to_string(lefteye) + "," + std::to_string(righteye) + "," + std::to_string(mouse) + "," 
			+ std::to_string(x) + "," + std::to_string(y) + "," + std::to_string(z) << std::endl; // 使用與cout同樣的方式進行寫入
			
			fout.close();  // 執行完操作後關閉檔案控制代碼
		}
		else {
			std::cout << "Create fall!!" << std::endl;
		}
