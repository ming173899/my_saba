...
namespace saba
{
	...
	public:
		...
		void Initialize(const glm::vec3& center, float radius, int cameraModes);
		void Initialize(const glm::vec3& center, glm::vec3& eye, float nearClip, float farClip, float radius);
		void Initialize(float left, float right, float button, float top);
		...
		void TwoPointMove(float x, float y);

		float GetOrthoLeft() const;
		float GetOrthoRight() const;
		float GetOrthoButton() const;
		float GetOrthoTop() const;

		glm::vec3 GetCenterPostion() const;
